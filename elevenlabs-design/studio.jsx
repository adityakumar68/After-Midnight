/* STUDIO SCENE — /studio
   States: idle | incoming | conversation | song_select | song_playing | caller_reaction
*/

const { useState, useEffect, useRef, useCallback } = React;

const DEFAULT_CALLER = { name: "MIRA", age: 24, location: "PORTLAND, OR", persona: "" };
const DEFAULT_SONGS = [
  { id: "country",  title: "Dusty Country",        vibe: "DUSTY COUNTRY" },
  { id: "synth",    title: "Synthwave Heartbreak", vibe: "SYNTHWAVE HEARTBREAK" },
  { id: "lullaby",  title: "Lullaby Piano",        vibe: "LULLABY PIANO" },
];
...
function Studio({
  ...
  onPushToTalkEnd,
  onPickSong,
  onAdvance, // demo helper
}) {
  // Internal demo state machine if no controller is wired
  const [callState, setCallState] = useState(extState || "idle");
  useEffect(() => { if (extState) setCallState(extState); }, [extState]);

  // Simulated VU levels when not provided
  ...
  useEffect(() => {
    ...
    const tick = (t) => {
      ...
      // caller VU: lively during conversation, low during song
      let cv = 0;
      if (isConv) {
      ...
      // DJ VU: only when push to talk down
      let dv = ptDownRef.current ? 0.4 + 0.5 * Math.abs(Math.sin(ts * 7.3)) : 0;
  ...
  useEffect(() => {
    const onDown = (e) => {
      ...
      if (e.code === "Enter" && callState === "incoming") {
        ...
        if (songs[idx]) handlePickSong(songs[idx].id);
      }
    };
    const onUp = (e) => {
  ...
  /* ---------- handlers (with internal demo flow) ---------- */
  const [pickedSong, setPickedSong] = useState(null);

  const handleAnswer = () => {
    onAnswer && onAnswer();
    setCallState("conversation");
  };
  const handlePtDown = () => {
    ptDownRef.current = true;
    setPtDown(true);
  ...
  const handlePickSong = (id) => {
    const s = songs.find(x => x.id === id);
    setPickedSong(s);
    onPickSong && onPickSong(id);
    setCallState("song_playing");
  };

  /* ---------- DEMO controller (cycles states for showcase) ---------- */
  const cycleState = () => {
    const order = ["idle", "incoming", "conversation", "song_select", "song_playing", "caller_reaction"];
    const i = order.indexOf(callState);
    const next = order[(i + 1) % order.length];
    if (next === "song_playing") setPickedSong(songs[1]);
    setCallState(next);
  };

  /* ---------- UI ---------- */
  const conversational = callState === "conversation";
  ...
  // Scale 1280x800 canvas to fit any viewport
    ...
    <Booth>
      <div className="fit-stage">
        ...
        <div style={{
          ...
          {/* ------- INCOMING PHONE (overlay, top-right) ------- */}
          {callState === "incoming" && (
            <IncomingPhone onAnswer={handleAnswer} />
          )}

          {/* ------- DEMO STATE PILL ------- */}
          <DemoStatePill state={callState} onCycle={cycleState} />
        </div>
        </div>
      </div>
    </Booth>
...
function DeckStrip({ callState, caller, callIndex, callTotal, clock }) {
  ...
  else crtLine = `CALL 0${callIndex} / 0${callTotal}  •  ${caller.name}, ${caller.age}  •  ${caller.location}  •  ${clock}`;
...
function Console({ callState, cv, dv, ptDown, onPtDown, onPtUp }) {
  ...
  return (
    <div style={{
      ...
      boxShadow: `
        inset 0 1px 0 rgba(255,179,71,0.10),
      ...
      backgroundImage: `
        repeating-linear-gradient(90deg, rgba(0,0,0,0.0) 0, rgba(0,0,0,0.0) 60px, rgba(0,0,0,0.15) 60px, rgba(0,0,0,0.15) 61px),
        linear-gradient(180deg, #3a2515 0%, #2a1a0f 60%, #1f130a 100%)
      `,
    }}>
      <ChannelStrip
        side="left"
...
function ChannelStrip({ side, label, sublabel, level, callState }) {
  ...
  return (
    <div style={{
      ...
      {/* speaker grille OR mic */}
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", width: "100%" }}>
        {isCaller
          ? <SpeakerGrille active={callState === "conversation" || callState === "caller_reaction"} level={level} />
          : <BroadcastMic active={callState === "conversation"} level={level} />
        }
      </div>
...
function SpeakerGrille({ active, level }) {
  const ripples = [0, 1, 2];
  return (
    <div style={{
      ...
      boxShadow: `
        ...
        inset 0 -2px 4px rgba(255,179,71,0.06),
...
function BroadcastMic({ active, level }) {
  // Shure SM7B-ish silhouette, tilted 15deg
  return (
    <div style={{
...
function LabelPlate({ primary, secondary }) {
  return (
    <div style={{
      ...
      {secondary && <div className="font-plex" style={{
...
function RecordSleeve({ song, angle, disabled, highlight, onClick, shortcut }) {
  ...
  return (
    ...
    >
      <div style={{
        ...
        <div style={{
          position: "absolute", inset: 0,
          backgroundImage: "url(\"data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='100' height='100'><filter id='n'><feTurbulence baseFrequency='0.7'/></filter><rect width='100%' height='100%' filter='url(%23n)' opacity='0.35'/></svg>\")",
...
function Turntable({ visible, song }) {
  return (
    <div style={{
      background: "linear-gradient(180deg, #2A1A0F 0%, #1a0f08 100%)",
      borderRadius: 10,
      ...
      <svg width="100" height="170" style={{ position: "absolute", right: 16, top: 12 }}>
        ...
        <line x1="84" y1="22" x2="20" y2={visible ? 110 : 60} stroke="url(#armG)" strokeWidth="3"
              style={{ transition: "y2 800ms ease" }} />
        <circle cx="20" cy={visible ? 110 : 60} r="4" fill="#222" stroke="#666"
                style={{ transition: "cy 800ms ease" }} />
      </svg>
...
/* =====================================================================
   INCOMING PHONE (top-right)
   ===================================================================== */
function IncomingPhone({ onAnswer }) {
  return (
    <button
      onClick={onAnswer}
...
/* =====================================================================
   DEMO CONTROLLER PILL
   ===================================================================== */
function DemoStatePill({ state, onCycle }) {
  return (
    <div style={{
      position: "absolute",
      bottom: 14,
      ...
      <span className="font-mono tracked">DEMO STATE</span>
      <button
        onClick={onCycle}
      ...
      >{state.toUpperCase().replace("_", " ")} → NEXT</button>
    </div>
  );
}
